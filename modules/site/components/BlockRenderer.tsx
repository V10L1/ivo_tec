...
import { PageBlock, ThemeSettings, Viewport, TextStyles, ContainerStyles, FixedContainerPosition, StyledText, ColorStyleValue, ThemeColorKey } from '../../../types';
import { resolveColor } from '../PublicSite';
...
const createTextStyle = (textStyles: TextStyles | undefined, theme: ThemeSettings, type: 'heading' | 'body' = 'body', textOpacity: number = 1): React.CSSProperties => { if (!textStyles) return {}; const font = type === 'heading' ? theme?.headingFont : theme?.bodyFont; return { color: resolveColor(textStyles.textColor, theme), textAlign: textStyles.textAlign, fontWeight: textStyles.fontWeight, fontStyle: textStyles.fontStyle, fontFamily: font || textStyles.fontFamily, fontSize: textStyles.fontSize ? `${textStyles.fontSize}px` : undefined, opacity: textOpacity, }; };
...
const defaultContainerStyles: ContainerStyles = { backgroundColor: { type: 'global', value: 'surface' }, backgroundOpacity: 1, textOpacity: 1, borderRadius: 'medium', zIndex: 0 };
...
const BlockRenderer: React.FC<BlockRendererProps> = ({ block, theme, viewport, isEditing = false, onToggleContainer = () => {} }) => {
    ...
    const resolvedBgColor = resolveColor(styles.backgroundColor, theme);
    const inlineStyle: React.CSSProperties = { backgroundColor: styles.backgroundOpacity !== 1 ? hexToRgba(resolvedBgColor, styles.backgroundOpacity) : resolvedBgColor, };
    const pointerEventsClass = isEditing ? 'pointer-events-none' : '';
    const animationClass = !isEditing && block.animation.type !== 'none' ? 'opacity-0' : '';
    
    switch (block.type) {
        case 'hero': return ( <div style={inlineStyle} className={`w-full h-full flex flex-col p-4 text-center items-center justify-center ${borderRadiusClass} ${animationClass}`}> <RenderText tag="h1" content={block.content.title} theme={theme} textOpacity={styles.textOpacity} isHeading className="text-4xl md:text-5xl font-extrabold mb-4" /> <RenderText tag="p" content={block.content.subtitle} theme={theme} textOpacity={styles.textOpacity} className="text-md md:text-lg max-w-2xl mx-auto mb-6" /> {block.content.ctaEnabled && ( <a href={block.content.ctaLink} className={`bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-3 px-8 rounded-full text-lg transition-transform transform hover:scale-105 ${pointerEventsClass}`} style={{ opacity: styles.textOpacity, backgroundColor: theme.primaryColor }}> {block.content.ctaText} </a> )} </div> );
        case 'text': return ( <div style={inlineStyle} className={`w-full h-full flex flex-col p-4 text-left ${borderRadiusClass} ${animationClass}`}> <RenderText tag="h2" content={block.content.heading} theme={theme} textOpacity={styles.textOpacity} isHeading className="text-3xl font-bold mb-4" /> <RenderText tag="div" content={block.content.body} theme={theme} textOpacity={styles.textOpacity} className="whitespace-pre-wrap leading-relaxed" /> </div> );
        ...
        case 'button': {
            ...
            const commonButtonClasses = `font-bold py-3 px-8 inline-block transition-colors ${borderRadiusClass}`;
        ...
        case 'menu': return ( <nav style={inlineStyle} className={`w-full h-full flex flex-row items-center justify-center gap-6 ${borderRadiusClass} ${pointerEventsClass} ${animationClass}`}> {block.content.items.map(item => ( <a key={item.id} href={item.link} className={`font-medium transition-colors ${pointerEventsClass}`} style={{ opacity: styles.textOpacity, color: theme.textSecondaryColor }}> {item.label} </a> ))} </nav> );
        ...
        case 'divider': return <div className={`flex items-center justify-center w-full h-full ${animationClass}`}><hr className="w-full" style={{borderColor: resolvedBgColor, opacity: styles.backgroundOpacity}}/></div>;
