...
import { Page, SiteData, PageBlock, SiteSettings, HeroBlockContent, TextBlockContent, ImageBlockContent, ButtonBlockContent, MenuBlockContent, VideoBlockContent, MenuItem, GridSettings, BlockLayout, ContainerStyles, TextStyles, StyledText, FixedContainer, FixedContainerPosition, FixedContainers } from '../../types';
import { useAuth } from '../../contexts/AuthContext';
import { PlusCircleIcon, SettingsIcon, Trash2Icon, MotorcycleIcon, TypeIcon, ImageIcon, CodeIcon, ChevronLeftIcon, ChevronRightIcon, SaveIcon, ArrowLeftIcon, FilePlusIcon, EditIcon, LayoutIcon, MenuIcon, PointerIcon, AlignStartVerticalIcon, AlignCenterVerticalIcon, AlignEndVerticalIcon, AlignStartHorizontalIcon, AlignCenterHorizontalIcon, AlignEndHorizontalIcon, GridIcon, VideoIcon, DividerIcon, SparklesIcon, BringToFrontIcon, SendToBackIcon, BoldIcon, ItalicIcon, AlignLeftIcon, AlignCenterIcon, AlignRightIcon, AlignJustifyIcon, SquareIcon, RoundedSquareIcon, CircleIcon, PanelTopIcon, PanelLeftIcon, PanelRightIcon, PanelBottomIcon, PanelOpenIcon, PanelCloseIcon } from '../../components/icons/Icons';
...
const createNewBlock = (type: PageBlock['type']): PageBlock => {
    ...
    switch (type) {
      case 'hero': return { ...baseBlock, type, content: { title: { text: 'Título do Herói', styles: {...defaultTextStyles, fontSize: 48, textAlign: 'center', fontWeight: 'bold'}}, subtitle: { text: 'Subtítulo atraente.', styles: {...defaultTextStyles, fontSize: 20, textAlign: 'center'}}, ctaText: 'Saiba Mais', ctaLink: '#', ctaEnabled: true } };
      ...
      default: return { ...baseBlock, type: 'text', content: { heading: { text: 'Bloco Desconhecido', styles: defaultTextStyles }, body: { text: '', styles: defaultTextStyles } } };
    }
};
...
const EditorBlockRenderer: React.FC<{ block: PageBlock }> = ({ block }) => { const commonClasses = "w-full h-full flex flex-col p-2"; const scaleText = "text-xs md:text-sm"; const styles = { ...defaultContainerStyles, ...block.styles }; const borderRadiusClass = getBorderRadiusClass(styles.borderRadius); const inlineStyle: React.CSSProperties = { backgroundColor: hexToRgba(styles.backgroundColor || '#1e293b', styles.backgroundOpacity), zIndex: styles.zIndex, }; const getYouTubeEmbedUrl = (url: string, autoplay?: boolean, controls?: boolean) => { try { let videoId; if (url.includes('youtube.com/watch')) { videoId = new URL(url).searchParams.get('v'); } else if (url.includes('youtu.be/')) { videoId = new URL(url).pathname.split('/').pop(); } if (!videoId) return null; const embedUrl = new URL(`https://www.youtube.com/embed/${videoId}`); if (autoplay) { embedUrl.searchParams.set('autoplay', '1'); embedUrl.searchParams.set('mute', '1'); } if (controls === false) { embedUrl.searchParams.set('controls', '0'); } return embedUrl.toString(); } catch { return null; } }; switch (block.type) { case 'hero': return ( <div style={inlineStyle} className={`${commonClasses} text-center items-center justify-center ${borderRadiusClass}`}> <h1 className="font-extrabold mb-1" style={createTextStyle(block.content.title.styles, styles.textOpacity)}>{block.content.title.text}</h1> <p className={`max-w-2xl mx-auto mb-2`} style={createTextStyle(block.content.subtitle.styles, styles.textOpacity)}>{block.content.subtitle.text}</p> {block.content.ctaEnabled && <div className="bg-cyan-600 text-white font-bold py-1 px-3 rounded-full text-xs" style={{opacity: styles.textOpacity}}>{block.content.ctaText}</div>} </div> ); case 'text': return ( <div style={inlineStyle} className={`${commonClasses} text-left overflow-hidden ${borderRadiusClass}`}> <h2 className="font-bold mb-1 truncate" style={createTextStyle(block.content.heading.styles, styles.textOpacity)}>{block.content.heading.text}</h2> <p className={`whitespace-pre-wrap leading-relaxed`} style={createTextStyle(block.content.body.styles, styles.textOpacity)}>{block.content.body.text}</p> </div> ); case 'image': return <img src={block.content.imageUrl} alt={block.content.altText} className={`w-full h-full object-cover ${borderRadiusClass}`} style={{opacity: styles.backgroundOpacity}}/>; case 'button': return <div className={`${commonClasses} items-center justify-center`}><div className={`text-white font-bold py-2 px-4 inline-block ${borderRadiusClass}`} style={{...inlineStyle, ...createTextStyle(block.content.text.styles, styles.textOpacity)}}>{block.content.text.text}</div></div>; case 'menu': return <nav style={inlineStyle} className={`${commonClasses} flex-row items-center justify-center gap-2 ${borderRadiusClass}`}><p className='text-xs' style={{opacity: styles.textOpacity}}>Menu</p>{block.content.items.map(item => (<div key={item.id} className={`font-medium ${scaleText}`} style={{opacity: styles.textOpacity}}>{item.label}</div>))}</nav>; case 'video': const embedUrl = getYouTubeEmbedUrl(block.content.videoUrl, block.content.autoplay, block.content.controls); return ( <div className={`w-full h-full bg-slate-900 ${borderRadiusClass} flex items-center justify-center text-slate-500 relative pointer-events-none`}> <VideoIcon className="w-1/3 h-1/3"/> {embedUrl && <iframe src={embedUrl} className="absolute inset-0 w-full h-full" title="preview"></iframe>} </div> ); case 'divider': return <div className="flex items-center justify-center w-full h-full"><hr className="w-full border-slate-700" style={{borderColor: styles.backgroundColor, opacity: styles.backgroundOpacity}}/></div>; case 'spacer': return <div className={`w-full h-full bg-slate-700/20 ${borderRadiusClass}`} style={inlineStyle}></div>; default: return <div className="p-4 bg-red-900 rounded-lg">Bloco desconhecido</div>; } };
const Block: React.FC<{ block: PageBlock; isSelected: boolean; onMouseDown: (e: React.MouseEvent, block: PageBlock) => void; onResizeStart: (e: React.MouseEvent, block: PageBlock, direction: string) => void; }> = ({ block, isSelected, onMouseDown, onResizeStart }) => { const layout = block.layout.desktop; const borderRadiusClass = getBorderRadiusClass(block.styles?.borderRadius); const blockStyle = { gridColumn: `${layout.colStart} / ${layout.colEnd}`, gridRow: `${layout.rowStart} / ${layout.rowEnd}`, alignSelf: layout.alignSelf, justifySelf: layout.justifySelf, zIndex: block.styles?.zIndex || 'auto', }; const resizeHandles = ['ne', 'se', 'sw', 'nw', 'n', 'e', 's', 'w']; return ( <div style={blockStyle} className={`relative group transition-shadow duration-200 ${isSelected ? 'shadow-2xl shadow-cyan-500/30' : ''}`} onMouseDown={(e) => onMouseDown(e, block)} > <div className={`absolute inset-0 ring-2 pointer-events-none transition-all duration-200 ${borderRadiusClass} ${isSelected ? 'ring-cyan-500' : 'ring-transparent group-hover:ring-cyan-500/50'}`}></div> <div className={`w-full h-full overflow-hidden pointer-events-none ${borderRadiusClass}`}> <EditorBlockRenderer block={block} /> </div> {isSelected && resizeHandles.map(dir => ( <div key={dir} className={`absolute w-3 h-3 bg-cyan-500 border-2 border-slate-900 rounded-full resize-handle-${dir} cursor-${dir}-resize z-50`} onMouseDown={(e) => { e.stopPropagation(); onResizeStart(e, block, dir); }} ></div> ))} </div> ); };
...
const SiteEditor: React.FC<{ onBack: () => void }> = ({ onBack: onBackToDashboard }) => {
  ...
  const canvasRefs = {
    main: useRef<HTMLDivElement>(null), footer: useRef<HTMLDivElement>(null), top: useRef<HTMLDivElement>(null),
    left: useRef<HTMLDivElement>(null), right: useRef<HTMLDivElement>(null), bottom: useRef<HTMLDivElement>(null),
  };

  const handleFeedback = (type: 'error' | 'success', message: string) => { setFeedback({ type, message }); setTimeout(() => setFeedback(null), 4000); };
  const fetchPages = useCallback(async () => { setStatus('loading'); try { const response = await fetch('/api/site/pages', { headers: { 'Authorization': `Bearer ${token}` } }); if (!response.ok) throw new Error('Falha ao buscar páginas'); const data = await response.json(); setPages(data); setStatus('idle'); } catch (error) { console.error(error); setStatus('error'); } }, [token]);
  useEffect(() => { if (view === 'list') fetchPages(); }, [view, fetchPages]);
  useEffect(() => { document.title = view === 'editor' && editingPage?.content?.settings.brandName ? `Editando: ${editingPage.title} | ${editingPage.content.settings.brandName}` : 'Painel de Administração Modular'; return () => { document.title = 'Painel de Administração Modular'; }; }, [view, editingPage]);

  const handleEditPage = async (page: Page) => { setStatus('loading'); try { const response = await fetch(`/api/site/pages/${page.id}`, { headers: { 'Authorization': `Bearer ${token}` } }); if (!response.ok) throw new Error('Falha ao carregar dados.'); const fullPageData: Page = await response.json(); const content = (fullPageData.content || {}) as any; const validatedContent: SiteData = { settings: content?.settings || defaultPageContent.settings, gridSettings: content?.gridSettings || defaultPageContent.gridSettings, fixedContainers: content?.fixedContainers || defaultPageContent.fixedContainers, mainBlocks: content?.mainBlocks || [], footerBlocks: content?.footerBlocks || [], }; fullPageData.content = validatedContent; setEditingPage(fullPageData); setSavedPage(JSON.parse(JSON.stringify(fullPageData))); setView('editor'); setSelectedBlock(null); setActiveTab('components'); } catch (error: any) { handleFeedback('error', error.message || 'Não foi possível carregar a página.'); } finally { setStatus('idle'); } };
  const handleSaveChanges = async () => { if (!editingPage) return; setStatus('saving'); try { const response = await fetch(`/api/site/pages/${editingPage.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ title: editingPage.title, slug: editingPage.slug, is_published: editingPage.is_published, content: editingPage.content }) }); if (!response.ok) throw new Error((await response.json()).message || 'Falha ao salvar'); const updatedPage = await response.json(); setSavedPage(JSON.parse(JSON.stringify(updatedPage))); setEditingPage(updatedPage); handleFeedback('success', 'Salvo com sucesso!'); } catch (error: any) { handleFeedback('error', error.message || 'Falha ao salvar!'); } finally { setStatus('idle'); } };
  const handleCreatePage = async (e: React.FormEvent) => { e.preventDefault(); setStatus('saving'); try { const response = await fetch('/api/site/pages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ title: newPageData.title, slug: newPageData.slug }) }); if (!response.ok) throw new Error((await response.json()).message || 'Falha ao criar'); const newPage = await response.json(); setNewPageData({ title: '', slug: '' }); await handleEditPage(newPage); } catch (error: any) { handleFeedback('error', error.message || 'Erro ao criar'); } finally { setStatus('idle'); } };
  const handleDeletePage = async (pageId: string) => { if (!window.confirm("Tem certeza?")) return; setStatus('deleting'); try { const response = await fetch(`/api/site/pages/${pageId}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } }); if (!response.ok) throw new Error((await response.json()).message || 'Falha ao excluir'); fetchPages(); } catch (error: any) { handleFeedback('error', error.message || 'Falha ao excluir'); } finally { setStatus('idle'); } };
  // FIX: Rewrote handleDeleteBlock to be type-safe by directly filtering known block arrays instead of iterating over object keys. This resolves a latent type error.
  const handleDeleteBlock = () => { if (!selectedBlock) return; updateEditingPage(draft => { if (!draft.content) return; draft.content.mainBlocks = draft.content.mainBlocks.filter(b => b.id !== selectedBlock.id); draft.content.footerBlocks = draft.content.footerBlocks.filter(b => b.id !== selectedBlock.id); if (draft.content.fixedContainers) { (Object.keys(draft.content.fixedContainers) as FixedContainerPosition[]).forEach(pos => { draft.content.fixedContainers[pos].blocks = draft.content.fixedContainers[pos].blocks.filter(b => b.id !== selectedBlock.id); }); } }); setSelectedBlock(null); };
  
  const updateEditingPage = (updater: (draft: Page) => void) => { setEditingPage(prev => { if (!prev) return null; const draft = JSON.parse(JSON.stringify(prev)); updater(draft); return draft; }); };
  ...
  const updateBlock = (updatedBlock: PageBlock) => { updateEditingPage(draft => { if (!draft.content) return; const blockArrays = Object.values(getBlockArraysForUpdate(draft.content)); for (const arr of blockArrays) { const index = arr.findIndex(b => b.id === updatedBlock.id); if (index !== -1) { arr[index] = updatedBlock; setSelectedBlock(updatedBlock); break; } } }); };
  const handleZIndexChange = (direction: 'front' | 'back') => { if (!selectedBlock) return; updateEditingPage(draft => { if (!draft.content) return; const allBlocks = Object.values(getBlockArraysForUpdate(draft.content)).flat(); const zIndexes = allBlocks.map(b => b.styles?.zIndex || 0); const maxZ = Math.max(0, ...zIndexes); const minZ = Math.min(0, ...zIndexes); const newZ = direction === 'front' ? maxZ + 1 : minZ - 1; updateBlock({ ...selectedBlock, styles: { ...(selectedBlock.styles || {}), zIndex: newZ } }); }); };
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
        ...
        let currentContext: EditorContext = 'main';
        ...
        const cellWidth = (canvasEl.getBoundingClientRect().width - (grid.columns - 1) * grid.gap) / grid.columns;
        const cellHeight = grid.rowHeight;
        const dx = e.clientX - interactionState.initialMouse.x; const dy = e.clientY - interactionState.initialMouse.y;
        const dCol = Math.round(dx / (cellWidth + grid.gap)); const dRow = Math.round(dy / (cellHeight + grid.gap));
        
        const { initialLayout } = interactionState; let newLayout = { ...initialLayout };
        if (interactionState.type === 'move' || interactionState.type === 'new') {
            newLayout.colStart = Math.max(1, initialLayout.colStart + dCol); newLayout.rowStart = Math.max(1, initialLayout.rowStart + dRow);
            newLayout.colEnd = newLayout.colStart + (initialLayout.colEnd - initialLayout.colStart); newLayout.rowEnd = newLayout.rowStart + (initialLayout.rowEnd - initialLayout.rowStart);
        ...
        if (interactionState.type === 'new') { setInteractionState({...interactionState, block: updatedBlock, targetContext: currentContext }); } else { updateBlock(updatedBlock); }
    };
    ...
    const handleMouseUp = () => {
        ...
        if (interactionState?.type === 'new') {
            ...
            updateEditingPage(draft => {
                ...
                blockArrays[finalContext].push(interactionState.block);
            });
            setSelectedBlock(interactionState.block); setSelectedContainerId(null); setActiveTab('inspector');
        }
  ...
  // ----- RENDERERS -----
  const renderListView = () => ( <div className="p-6"> <div className="flex justify-between items-center mb-6"> <h2 className="text-2xl font-bold text-slate-100">Gerenciador de Páginas</h2> <button onClick={() => setView('create')} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2 px-4 rounded-lg flex items-center gap-2"><FilePlusIcon className="w-5 h-5"/> Criar Página</button> </div> {status === 'loading' && <div className="text-center">Carregando...</div>} {status === 'error' && <div className="text-center text-red-400">Falha ao carregar.</div>} {status !== 'loading' && ( <div className="overflow-x-auto bg-slate-800/50 rounded-lg border border-slate-800"> <table className="min-w-full"> <thead><tr className="border-b border-slate-700"><th className="p-3 text-left text-sm font-semibold text-slate-400">Título</th><th className="p-3 text-left text-sm font-semibold text-slate-400">URL</th><th className="p-3 text-left text-sm font-semibold text-slate-400">Status</th><th className="p-3 text-left text-sm font-semibold text-slate-400">Ações</th></tr></thead> <tbody> {pages.map(page => ( <tr key={page.id} className="border-b border-slate-800 hover:bg-slate-800/50"> <td className="p-3 text-sm font-medium">{page.title} {page.is_homepage && <span className="text-xs bg-cyan-500/20 text-cyan-300 px-2 py-1 rounded-full ml-2">Página Inicial</span>}</td> <td className="p-3 text-sm text-slate-400 font-mono">/{page.slug}</td> <td className="p-3 text-sm">{page.is_published ? <span className="text-green-400">Publicada</span> : <span className="text-yellow-400">Rascunho</span>}</td> <td className="p-3 text-sm flex items-center gap-4"> <button onClick={() => handleEditPage(page)} className="text-cyan-400 hover:text-cyan-300 flex items-center gap-1"><EditIcon className="w-4 h-4"/> Editar</button> <button onClick={() => handleDeletePage(page.id)} className="text-red-500 hover:text-red-400 flex items-center gap-1"><Trash2Icon className="w-4 h-4"/> Excluir</button> </td> </tr> ))} </tbody> </table> </div> )} </div> );
  const renderCreateView = () => ( <div className="p-6"> <button onClick={() => setView('list')} className="flex items-center gap-2 mb-4 text-slate-400 hover:text-white"><ArrowLeftIcon className="w-4 h-4" /> Voltar</button> <h2 className="text-2xl font-bold text-slate-100 mb-6">Criar Nova Página</h2> <form onSubmit={handleCreatePage} className="max-w-md space-y-4"> <div><label className="block text-sm font-medium text-slate-400 mb-1">Título</label><input type="text" value={newPageData.title} onChange={e => setNewPageData({...newPageData, title: e.target.value})} required className="w-full bg-slate-900 border border-slate-700 rounded-md p-2" /></div> <div><label className="block text-sm font-medium text-slate-400 mb-1">URL (Slug)</label><input type="text" value={newPageData.slug} onChange={e => setNewPageData({...newPageData, slug: e.target.value.toLowerCase().replace(/\s+/g, '-')})} required className="w-full bg-slate-900 border border-slate-700 rounded-md p-2" /><p className="text-xs text-slate-500 mt-1">Apenas letras, números e hifens.</p></div> <div className="pt-2"><button type="submit" disabled={status === 'saving'} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2 px-4 rounded-lg">{status === 'saving' ? 'Criando...' : 'Criar e Editar'}</button></div> </form> </div> );
  ...
  const renderEditorView = () => {
    ...
    const { gridSettings, fixedContainers } = editingPage.content;
    
    const FixedContainerEditor: React.FC<{position: FixedContainerPosition}> = ({ position }) => {
        const container = fixedContainers[position];
        const isHorizontal = position === 'top' || position === 'bottom';
        if (!container.enabled) return null;
        
        const style: React.CSSProperties = isHorizontal ? { height: `${container.size}px` } : { width: `${container.size}px` };
        
        return (
            <div className="relative flex-shrink-0 p-2 bg-slate-800/50 border-slate-700" style={style}>
                 <div className="absolute inset-0 border-2 border-transparent hover:border-cyan-500/50 pointer-events-auto" onClick={() => { setSelectedContainerId(position); setSelectedBlock(null); setActiveTab('inspector'); }}></div>
                 <GridCanvas blocks={container.blocks} context={position} />
                 <span className="absolute top-0 left-1 text-xs text-slate-600 capitalize">{position}</span>
            </div>
        );
    };

    const GridCanvas = ({ blocks, context }: { blocks: PageBlock[], context: EditorContext }) => {
        const gridStyle = { display: 'grid', gridTemplateColumns: `repeat(${gridSettings.desktop.columns}, 1fr)`, gridAutoRows: `${gridSettings.desktop.rowHeight}px`, gap: `${gridSettings.desktop.gap}px` };
        return ( <div ref={canvasRefs[context]} style={gridStyle} className="relative bg-slate-900/50 rounded-lg border border-dashed border-slate-700 min-h-[100px] h-full w-full p-2" onMouseDown={(e) => { if (e.target === e.currentTarget) { setSelectedBlock(null); setSelectedContainerId(null); setActiveTab('components'); } }}> {blocks.map(block => <Block key={block.id} block={block} isSelected={selectedBlock?.id === block.id} onMouseDown={(e, b) => handleBlockMouseDown(e, b, context)} onResizeStart={(e, b, dir) => handleResizeStart(e, b, dir, context)} />)} </div> );
    };
    ...
    return (
      <div className="flex flex-row h-full w-full overflow-hidden bg-slate-900">
        ...
        <div className="flex-1 relative flex flex-col" style={pageStyle}>
            <button onClick={() => setIsPanelOpen(!isPanelOpen)} className={`absolute top-4 bg-slate-800 hover:bg-cyan-600 text-white p-2 rounded-r-lg z-30 transition-all ${isPanelOpen ? 'left-0' : 'left-0'}`}><ChevronLeftIcon className={`w-5 h-5 transition-transform ${isPanelOpen ? '' : 'rotate-180'}`}/></button>
            <div className="flex flex-col w-full h-full overflow-auto">
                <FixedContainerEditor position="top" />
                <div className="flex flex-row flex-grow min-h-0">
                    <FixedContainerEditor position="left" />
                    <main className="flex-grow p-4 space-y-4 flex flex-col">
                        <div className="p-2 flex-grow"><h3 className="text-center text-xs font-semibold uppercase text-slate-500 mb-2">Página Principal</h3><GridCanvas blocks={editingPage.content.mainBlocks} context="main" /></div>
                        <div className="p-2 border-t-2 border-dashed border-slate-700/50"><h3 className="text-center text-xs font-semibold uppercase text-slate-500 mb-2">Rodapé</h3><GridCanvas blocks={editingPage.content.footerBlocks} context="footer"/></div>
                    </main>
                    <FixedContainerEditor position="right" />
                </div>
                <FixedContainerEditor position="bottom" />
            </div>
        </div>
        
        {interactionState?.type === 'new' && ( <div className="fixed top-0 left-0 pointer-events-none z-50 opacity-80" style={{ transform: `translate(${interactionState.initialMouse.x + 10}px, ${interactionState.initialMouse.y + 10}px)`}}> <div className="flex items-center gap-3 p-3 bg-slate-700 rounded-md text-left"> <PointerIcon className="w-5 h-5 text-cyan-400"/> Novo Bloco: {interactionState.block.type} </div> </div> )}
      </div>
    );
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-900 text-slate-300 font-sans">
      <header className="bg-slate-900/80 backdrop-blur-sm z-40 border-b border-slate-800 flex items-center justify-between h-16 px-6 flex-shrink-0">
          <div className="flex items-center gap-4"> <button onClick={view === 'editor' || view === 'create' ? () => setView('list') : onBackToDashboard} className="bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white font-bold py-2 px-4 rounded-lg inline-flex items-center"><ArrowLeftIcon className="w-5 h-5 mr-2" /><span>{view === 'editor' || view === 'create' ? 'Páginas' : 'Painel'}</span></button> <h2 className="text-xl font-bold text-slate-100">{view === 'editor' && editingPage ? `Editando: ${editingPage.title}` : 'Gerenciador de Site'}</h2> </div>
      ...
      <div className="flex-1 relative overflow-hidden">
       ...
       <style>{`.resize-handle-ne { top: -6px; right: -6px; } .resize-handle-se { bottom: -6px; right: -6px; } .resize-handle-sw { bottom: -6px; left: -6px; } .resize-handle-nw { top: -6px; left: -6px; } .resize-handle-n { top: -6px; left: 50%; transform: translateX(-50%); cursor: n-resize; } .resize-handle-e { top: 50%; right: -6px; transform: translateY(-50%); cursor: e-resize; } .resize-handle-s { bottom: -6px; left: 50%; transform: translateX(-50%); cursor: s-resize; } .resize-handle-w { top: 50%; left: -6px; transform: translateY(-50%); cursor: w-resize; }`}</style>
    </div>
  );
};
