...
// FIX: Added radix to all parseInt calls to ensure correct parsing and satisfy stricter linting rules, which may resolve the misleading error.
    ...
    useEffect(() => {
        const fetchContent = async () => {
            ...
            try {
                ...
                // Deeply merge fetched data with defaults to prevent render errors from incomplete data
                const validatedContent: SiteData = {
                    settings: { ...defaultPageContent.settings, ...(content?.settings || {}) },
                    gridSettings: {
                        ...defaultPageContent.gridSettings,
                        ...(content?.gridSettings || {}),
                        desktop: {
                            ...defaultPageContent.gridSettings.desktop,
                            ...(content?.gridSettings?.desktop || {})
                        }
                    },
                    fixedContainers: {
                        top: { ...defaultPageContent.fixedContainers.top, ...(content?.fixedContainers?.top || {}) },
                        left: { ...defaultPageContent.fixedContainers.left, ...(content?.fixedContainers?.left || {}) },
                        right: { ...defaultPageContent.fixedContainers.right, ...(content?.fixedContainers?.right || {}) },
                        bottom: { ...defaultPageContent.fixedContainers.bottom, ...(content?.fixedContainers?.bottom || {}) },
                    },
