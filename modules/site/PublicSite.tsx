...
const getYouTubeEmbedUrl = (url: string, autoplay?: boolean, controls?: boolean) => { try { if (!url) return null; let videoId; const urlWithProtocol = url.startsWith('http') ? url : `https://${url}`; const parsedUrl = new URL(urlWithProtocol); if (parsedUrl.hostname.includes('youtube.com') && parsedUrl.searchParams.has('v')) { videoId = parsedUrl.searchParams.get('v'); } else if (parsedUrl.hostname.includes('youtu.be')) { videoId = parsedUrl.pathname.split('/').pop(); } if (!videoId) return null; const embedUrl = new URL(`https://www.youtube.com/embed/${videoId}`); if (autoplay) { embedUrl.searchParams.set('autoplay', '1'); embedUrl.searchParams.set('mute', '1'); } if (controls === false) { embedUrl.searchParams.set('controls', '0'); } return embedUrl.toString(); } catch (error) { return null; } };
const isObject = (item: any) => item && typeof item === 'object' && !Array.isArray(item);
const deepMerge = <T extends object>(target: T, ...sources: Partial<T>[]): T => { if (!sources.length) return target; const source = sources.shift(); if (isObject(target) && isObject(source)) { for (const key in source) { if (isObject((source as any)[key])) { if (!(target as any)[key]) Object.assign(target, { [key]: {} }); deepMerge((target as any)[key], (source as any)[key]); } else { Object.assign(target, { [key]: (source as any)[key] }); } } } return deepMerge(target, ...sources); };
    ...
    useEffect(() => {
        const fetchContent = async () => {
            ...
            try {
                ...
                const content = data.content as Partial<SiteData> | null;

                // --- ROBUST DATA SANITIZATION ---
                const sanitizeBlocks = (blocks: PageBlock[] = []): PageBlock[] => {
                    return blocks.map(block => {
                        // Create a fresh default block of the same type
                        const defaultBlock = createNewBlock(block.type);
                        // Deeply merge the block from the DB into the default structure
                        // This ensures all nested properties (like content.title.styles) exist
                        return deepMerge(defaultBlock, block);
                    });
                };
                
                const validatedContent: SiteData = deepMerge(
                    JSON.parse(JSON.stringify(defaultPageContent)), // Start with a clean default slate
                    (content || {}), // Merge the fetched content object
                    { // Explicitly overwrite block arrays with sanitized versions
                        mainBlocks: sanitizeBlocks(content?.mainBlocks),
                        footerBlocks: sanitizeBlocks(content?.footerBlocks),
                        fixedContainers: {
                            top: { blocks: sanitizeBlocks(content?.fixedContainers?.top?.blocks) },
                            left: { blocks: sanitizeBlocks(content?.fixedContainers?.left?.blocks) },
                            right: { blocks: sanitizeBlocks(content?.fixedContainers?.right?.blocks) },
                            bottom: { blocks: sanitizeBlocks(content?.fixedContainers?.bottom?.blocks) },
                        }
                    }
                );
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            if (interactionState.type === 'move' || interactionState.type === 'new') {
                ...
                newLayout.rowEnd = newLayout.rowStart + (initialLayout.rowEnd - initialLayout.rowEnd);
